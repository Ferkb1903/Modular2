  ...
  fHeterogeneityPhys1(nullptr), fHeterogeneityPhys2(nullptr)
{
 ...
 // OPTIMIZACIÓN: Reducido de 15 cm a 9 cm (18×18×18 cm total vs 30×30×30 original)
 // Beneficios: 4.6x más rápido, mejor relación señal/ruido, mantiene física
 fPhantomSizeX = 9.*cm;  // Phantom 18×18×18 cm total
 fPhantomSizeY = 9.*cm;
 fPhantomSizeZ = 9.*cm;
 
 // Define the sizes of the World volume containing the phantom
 // OPTIMIZACIÓN: Reducido proporcionalmente al phantom optimizado
 fWorldSizeX = 30.*cm;  // ~3.3 times phantom size (era 50 cm)
 fWorldSizeY = 30.*cm;  // Suficiente margen para visualización
 fWorldSizeZ = 30.*cm;  // Proporciones más eficientes
...
void BrachyDetectorConstruction::ConstructPhantom()
{
  ...
  // Construct heterogeneities if enabled
  ConstructHeterogeneousPhantom();
}
...
void BrachyDetectorConstruction::EnableHeterogeneities(G4bool enable)
{
  fEnableHeterogeneities = enable;
  G4cout << "Heterogeneities " << (enable ? "enabled" : "disabled") << G4endl;
}

void BrachyDetectorConstruction::SetHeterogeneityType(G4String heterogeneityType)
{
  fHeterogeneityType = heterogeneityType;
...
void BrachyDetectorConstruction::ConstructHeterogeneousPhantom()
{
  ...
  // Define materials for heterogeneities
  G4Material* heterogeneityMaterial = nullptr;
  G4Colour heterogeneityColor;
  
  if (fHeterogeneityType == "bone") {
    // Cortical bone material (ICRP Publication 46)
    heterogeneityMaterial = nist->FindOrBuildMaterial("G4_BONE_CORTICAL_ICRP");
    heterogeneityColor = G4Colour(1.0, 1.0, 0.0); // Yellow
  } else if (fHeterogeneityType == "muscle") {
    // Muscle material
    heterogeneityMaterial = nist->FindOrBuildMaterial("G4_MUSCLE_SKELETAL_ICRP");
    heterogeneityColor = G4Colour(1.0, 0.0, 0.0); // Red
  } else if (fHeterogeneityType == "fat") {
    // Adipose tissue material
    heterogeneityMaterial = nist->FindOrBuildMaterial("G4_ADIPOSE_TISSUE_ICRP");
    heterogeneityColor = G4Colour(1.0, 0.5, 0.0); // Orange
  } else {
    // Default to lung tissue
    heterogeneityMaterial = nist->FindOrBuildMaterial("G4_LUNG_ICRP");
    heterogeneityColor = G4Colour(0.5, 0.5, 1.0); // Light blue
  }
  
  if (!heterogeneityMaterial) {
    G4cout << "WARNING: Heterogeneity material not found, using water instead" << G4endl;
  ...
  // We don't need the second cube anymore
  fHeterogeneityBox2 = nullptr;
  fHeterogeneityLog2 = nullptr;
  fHeterogeneityPhys2 = nullptr;
  
  // Set visualization attributes for the heterogeneity cube
  auto hetVisAtt = new G4VisAttributes(heterogeneityColor);
  hetVisAtt->SetVisibility(true);
  hetVisAtt->SetForceSolid(true);
  hetVisAtt->SetColor(heterogeneityColor.GetRed(), heterogeneityColor.GetGreen(), 
                      heterogeneityColor.GetBlue(), 0.8); // Semi-transparent
  
  fHeterogeneityLog1->SetVisAttributes(hetVisAtt);
  
  G4cout << "Single heterogeneity cube constructed:" << G4endl;
  G4cout << "  Phantom size: " << fPhantomSizeX*2./cm << " x " 
         << fPhantomSizeY*2./cm << " x " << fPhantomSizeZ*2./cm << " cm (water)" << G4endl;
